...
const PostCreatePage = () => {
  ...
  const filledEntries = entries.filter((entry) => entry.experiencePhotos.some((photo) => photo.previewUrl || photo.note.trim().length > 0)).length;
  ...
  const hasPendingUploads = entries.some(
    (entry) => entry.referenceImageUploadStatus === 'uploading' || entry.experiencePhotos.some((photo) => photo.uploadStatus === 'uploading'),
  );
  const hasFailedUploads = entries.some(
    (entry) => entry.referenceImageUploadStatus === 'error' || entry.experiencePhotos.some((photo) => photo.uploadStatus === 'error'),
  );
  const hasMissingUploadedPhotos = entries.some((entry) => entry.experiencePhotos.some((photo) => !hasUploadedUrl(photo.uploadedUrl)));
  ...
  const handleUpdatePhotoNote = (entryId, photoId, note) => {
    updateEntry(entryId, (entry) => ({
      ...entry,
      experiencePhotos: entry.experiencePhotos.map((photo) => (photo.id === photoId ? { ...photo, note } : photo)),
    }));
  };
      ...
      <section className={styles.pageShell}>
        ...
        <section className={styles.overviewCard}>
          ...
          <p className={styles.overviewText}>
            각 장소에서 대표 사진 1장과 감상을 입력하면, 등록 시 게시글 메타와 장소별 이미지 URL 및 노트를 JSON으로 함께 전송합니다.
          </p>
        ...
        <div className={styles.sceneSectionHeader}>
          <div className={styles.sectionTitleRow}>
            ...
            <p className={styles.sectionCaption}>루트 내 장면 사진은 자동으로 채우고, 각 장소마다 직접 찍은 대표 사진 1장과 감상을 남깁니다.</p>
          </div>

          <div className={styles.sectionAction}>
            <button type="button" className={styles.addPlaceButton} onClick={handleAddExtraPlace} disabled={!selectedRoute}>
              <PlusCircle size={16} />
              추가 장소 넣기
            </button>
          </div>
        ...
        {selectedRoute ? (
          ...
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyText}>루트를 먼저 선택하면 해당 장소 수만큼 입력 카드가 자동 생성됩니다.</p>
          </div>
        )}
      </section>
