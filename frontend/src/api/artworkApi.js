...
export const searchArtworkCandidates = async ({ query, mediaType = 'all' }) => {
  ...
  const localResults = toArray(
    await FetchJson(`/api/v1/main/artworks/search?keyword=${encodeURIComponent(normalizedQuery)}`, { method: 'GET' }),
  )
  ...
  return toArray(
    await FetchJson(`/api/v1/artworks/maps/search?query=${encodeURIComponent(normalizedQuery)}`, { method: 'GET' }),
  )
