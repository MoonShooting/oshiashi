...
public class BookmarkServiceImpl implements BookmarkService {
	...
	public BookmarkResponse createBookmark(String userId, BookmarkCreateRequest request) {
		...
		if (isDuplicate) {
			...
			log.debug("이미 존재하는 북마크입니다. 중단합니다. User: {}", userId);
			return null; // [보완] 실제로 로직을 중단시키기 위해 null 반환
		}

		// 3. 저장
		// [보완] getReferenceById는 ID가 null이면 예외가 발생하므로, 존재할 때만 호출하도록 삼항 연산자 사용
		BookmarkEntity bookmark = BookmarkEntity.builder()
				...
				.postImage(request.getPostImageId() != null ? postImageRepository.getReferenceById(request.getPostImageId()) : null)
				.route(request.getRouteId() != null ? routeRepository.getReferenceById(request.getRouteId()) : null)
				.build();

		BookmarkEntity savedBookmark = bookmarkRepository.save(bookmark);
		log.info("[Bookmark] 생성 완료 - ID: {}", savedBookmark.getBookmarkId());
